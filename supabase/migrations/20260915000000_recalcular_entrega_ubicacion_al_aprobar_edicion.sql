-- Bug real reportado por el equipo: al aprobar una edición que cambia
-- la dirección de ENTREGA, `direccion_entrega` (texto) se actualizaba
-- pero `entrega_ubicacion` (el punto geocodificado) se quedaba con las
-- coordenadas viejas. Como el precio del pedido se calcula en vivo con
-- ST_Distance(farmacia_ubicacion, entrega_ubicacion) — ver
-- app.obtener_datos_precio_pedido — el precio silenciosamente seguía
-- usando la distancia a la dirección de entrega anterior, aunque el
-- texto ya mostrara la nueva. Mismo patrón que ya se corrigió para
-- farmacia_ubicacion en 20260907020000; acá se completa el otro lado.

drop function if exists app.obtener_datos_geocodificacion_novedad_admin(uuid, uuid);
...
as $$
  select
    ...
    n.datos_propuestos->>'direccionEntrega',
    pp.ciudad,
    pp.departamento
...
drop function if exists app.aprobar_edicion_pedido_admin(uuid, uuid, double precision, double precision);
...
begin
  ...
  set
    ...
    entrega_ubicacion = case
      when v_novedad.datos_propuestos ? 'direccionEntrega'
        and p_entrega_lat is not null and p_entrega_lng is not null
      then public.st_setsrid(public.st_makepoint(p_entrega_lng, p_entrega_lat), 4326)::public.geography
      else entrega_ubicacion
    end,
    actualizado_en = now()
...
revoke all on function app.aprobar_edicion_pedido_admin(uuid, uuid, double precision, double precision, double precision, double precision) from public;
revoke all on function app.aprobar_edicion_pedido_admin(uuid, uuid, double precision, double precision, double precision, double precision) from anon;
revoke all on function app.aprobar_edicion_pedido_admin(uuid, uuid, double precision, double precision, double precision, double precision) from authenticated;
grant execute on function app.aprobar_edicion_pedido_admin(uuid, uuid, double precision, double precision, double precision, double precision) to mediruta_app;
