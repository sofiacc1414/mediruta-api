...
- La App (Flutter) y el Web **nunca** llaman a `supabase-js`/`supabase_flutter` para login — llaman a los endpoints de auth de la API (`POST /auth/registro`, `POST /auth/login`, `POST /auth/refrescar`, etc.) y guardan el JWT propio (en Flutter: almacenamiento seguro tipo `flutter_secure_storage`; en Web: `localStorage`, mismo flujo de body que la App — no cookie HttpOnly). Decisión explícita del equipo: se descartó la cookie HttpOnly cross-site porque Web (Vercel) y la API (Render) son dominios distintos sin un dominio propio en común, y los navegadores bloquean cada vez más agresivamente esa cookie por ser "de tercero" (rompía la persistencia de sesión al recargar la página) — se acepta el riesgo de XSS de `localStorage` a cambio de que la sesión sí persista sin depender de comprar un dominio propio.
...
- El **estado de cuenta** (`activa`, `bloqueada`, `desactivada`) y el **estado del rol** (`borrador` — solo DOMICILIARIO, todavía no envió su solicitud, invisible para el admin —, `habilitado`, `pendiente_validacion`, `rechazado`) son conceptos diferentes.
...
Registro público únicamente como `PACIENTE` o `DOMICILIARIO`. Un paciente nuevo recibe `PACIENTE` → `habilitado`. Un domiciliario nuevo recibe `DOMICILIARIO` → `borrador` (**no** `pendiente_validacion` directo — ver más abajo, hace falta enviar la solicitud a propósito) y, **opcionalmente** (`altaPaciente: boolean` en el registro, `false` si no se manda — no todos los domiciliarios van a querer ser también pacientes), `PACIENTE` → `habilitado`. `ADMINISTRADOR` y `ROOT` no tienen registro público; `ROOT` es una cuenta técnica/interna.

Una cuenta ya existente puede pedir el rol que le falta sin pasar por un registro nuevo (que además chocaría con el `UNIQUE` de correo): `POST /perfil/paciente/solicitar` agrega `PACIENTE` → `habilitado` sin validación; `POST /perfil/domiciliario/solicitar` agrega `DOMICILIARIO` → `borrador` y de ahí en más sigue el mismo camino que un domiciliario recién registrado (completar perfil/documentos, enviar solicitud, aprobación del admin) — ninguna otra función de HU-02 necesitó cambiar, ya exigían solo que la fila en `usuario_roles` existiera, nunca un `estado` puntual. Ambas son idempotentes (pedir un rol que ya se tiene no es un error). Además reusan lo que se solapa entre `perfil_paciente` y `perfil_domiciliario` (`direccion`, foto de cédula) en vez de pedirlo de nuevo — al domiciliario que pide Paciente solo le falta la fecha de nacimiento; al paciente que pide Domiciliario solo le falta vehículo y los otros 3 documentos.

**`borrador` → `pendiente_validacion` es un paso explícito**, `POST /perfil/domiciliario/enviar-solicitud` (`app.enviar_solicitud_domiciliario`) — mismo criterio que HU-03 (crear/completar una solicitud médica ≠ enviarla). Mientras está en `borrador`, el domiciliario puede completar su perfil/documentos con total libertad (HU-02 no distingue estados), pero es invisible para el admin (`app.listar_domiciliarios_pendientes` solo trae `pendiente_validacion`). Enviar exige los mismos 7 campos obligatorios que ya exigía `app.aprobar_domiciliario` — se piden ahora al enviar, no recién cuando el admin intenta aprobar.
